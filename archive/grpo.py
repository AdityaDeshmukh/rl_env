...
# --------------------------
# GRPO helper components
# --------------------------

def grpo_loss(advantages, per_token_logprobs, per_token_logprobs_old, beta, device):
    loss = 0
    n = 0
    for i in range(len(per_token_logprobs)):
        n += per_token_logprobs[i].shape[0]
        ratio = torch.exp(per_token_logprobs[i] - per_token_logprobs_old[i])
        term_1 = ratio * advantages[i]
        term_2 = ratio.clamp(1 - 0.2, 1 + 0.2) * advantages[i]
        curr_loss = -torch.min(term_1, term_2)
        loss += curr_loss.sum()
    return loss/n
...
if __name__ == "__main__":
    ...
    while global_step < args.total_timesteps:
        ...
        if iteration % 5 == 0:
            ...
            # agent_ref.eval()
            eval_return = evaluate_deterministic(args.env_id, agent, max_ep_len=500, device=device, episodes=5)
        ...
        for j in range(5):
            ...
            per_token_logprobs = [[] for _ in range(args.num_envs)]
            per_token_logprobs_old = [[] for _ in range(args.num_envs)]
            next_obs = torch.tensor(next_obs, device=device, dtype=torch.float32)
            for i in range(args.num_envs):
                for step in range(0, args.num_steps):
                    global_step += 1
                    action, logprob, entropy = agent.get_action(next_obs, num_envs=1, k=args.k, m=args.m, mix_eps=0, temp=args.temp)
                    # with torch.no_grad():
                    ...
                    per_token_logprobs_old[i].append(logprob[0].detach())
                    per_token_logprobs[i].append(logprob[0])
                    next_done = np.logical_or(termination, truncation)
                    next_obs = torch.Tensor(next_obs).to(device)
                    if next_done:
                        break
                per_token_logprobs[i] = torch.stack(per_token_logprobs[i])
                per_token_logprobs_old[i] = torch.stack(per_token_logprobs_old[i])
            
            advantages = sum_rewards - sum_rewards.mean()
            advantages = advantages / (advantages.std() + 1e-8)
            print("sum_rewards:", sum_rewards)
            loss += grpo_loss(advantages=advantages, per_token_logprobs=per_token_logprobs,
                               per_token_logprobs_old=per_token_logprobs_old, beta=args.beta, device=device)
