...
from utils import make_atari_clones
...
class Args:
    ...
    cuda: bool = True
    track: bool = False
    wandb_project_name: str = "atari_r_rebel"
...
class Agent(nn.Module):
    ...
    def get_action(self, x, num_envs, k=None, m=0, temp=1.0, mix_eps: Union[float, torch.Tensor] = 0, action=None):
        hidden = self.network(x / 255.0)
        logits = self.actor(hidden)/temp
...
if __name__ == "__main__":
    ...
    env.close()
    agent = Agent(act_dim=act_dim).to(device)
    agent_ref = Agent(act_dim=act_dim).to(device)
    ...
    while global_step < args.total_timesteps:
        ...
        loss = 0
        for j in range(5):
            envs, next_obs = make_atari_clones(args.env_id, args.num_envs, base_seed=np.random.randint(0, np.iinfo(np.int32).max), capture_video=args.capture_video, run_name=run_name)
